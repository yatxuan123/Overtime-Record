...
function App() {
  const { records, status: syncStatus, message: syncMessage, workerUrl, isPasswordConfigured, connect, disconnect, syncNow, replaceRecords, deleteRecord } = useCloudSync()
  ...
  const updateRecords = useCallback((next: OvertimeRecord[]) => { void replaceRecords(next) }, [replaceRecords])
  ...
  const handleDelete = useCallback((id: string) => { if (window.confirm('确定删除这条加班记录吗？')) { void deleteRecord(id); if (editingId === id) { setEditingId(null); setForm(emptyForm()) } setNotice('记录已删除'); window.setTimeout(() => setNotice(''), 2200) } }, [deleteRecord, editingId])
  ...
  return <div className="app-shell">
    ...
    <main className="page-container">
      <header className="topbar"><div className="brand-lockup"><div className="brand-mark"><ArrowUpRight size={19} /></div><div><span className="brand-name">加班有数</span><span className="brand-subtitle">OVERTIME LOG</span></div></div><div className="topbar-tools"><div className="topbar-date"><CalendarDays size={16} />{monthLabel(new Date())}</div><SyncControl status={syncStatus} message={syncMessage} workerUrl={workerUrl} isPasswordConfigured={isPasswordConfigured} onConnect={connect} onDisconnect={disconnect} onSync={syncNow} /></div></header>
