        ...
        <div className="record-list">
          {records.map((record) => (
            <article className="record-row" key={record.id}>
              ...
              <div className="record-main"><div className="record-title"><strong>{record.note || '未填写备注'}</strong><span className="record-badge">{record.tookTaxi ? '打车' : '自行回家'}</span></div><div className="record-meta"><span><Clock3 size={14} />{record.leaveTime} 下班 · {record.hours.toFixed(1)} 小时</span>{record.tookTaxi && <span><CarFront size={14} />¥{record.taxiCost.toFixed(0)}</span>}</div></div>
